...
func SendCode(c *gin.Context) {
	...
	if err = models.Redis.Set(context.Background(), define.RegisterPrefix+email, code, time.Second*time.Duration(define.ExpireTime)).Err(); err != nil {
		log.Println("[ERROR]:", err)
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
			"msg":  "系统错误",
...
func Register(c *gin.Context) {
	...
	//验证码是否正确
	r, err := models.Redis.Get(context.Background(), define.RegisterPrefix+email).Result()
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
			"msg":  "验证码错误",
